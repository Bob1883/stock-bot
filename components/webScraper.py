...
def get_historical_data(company, end_date, api, gathered_data, lookback_period=1.5):
    try:
        ...
        print_colored(f"historical data for {company} is ready", "yellow")

        return gathered_data
    
    except Exception as e:
        print_colored(f"historical data for {company} failed", "red")
...
def get_political_data(date, company, gathered_data):
    try:
        ...
        print_colored(f"political data for {company} is ready", "yellow")

        return gathered_data
    
    except:
        print_colored(f"political data for {company} failed", "red")
...
def get_bot_predictions(company, gathered_data):
    try:
        ...
        url = f"https://www.wallstreetzen.com/stocks/us/nasdaq/{symbol}"
        driver.get(url)
        soup = BeautifulSoup(driver.page_source, "html.parser")
        ...
        print_colored(f"bot predictions for {company} is ready", "yellow")

        return gathered_data
    except Exception as e:
        print_colored(f"bot predictions for {company} failed", "red")
        print(e)
